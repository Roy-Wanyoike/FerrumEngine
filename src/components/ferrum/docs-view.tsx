...
import {
  X, Search, Copy, Check, ChevronRight, Menu, ArrowLeft,
  Rocket, Layers, Puzzle, Sparkles, FileCode, Gauge, Shield,
  Terminal, Users, AlertTriangle, Info, Lightbulb, BookOpen, Palette,
} from "lucide-react";
import { toast } from "sonner";
import { docSections, navGroups, type DocBlock, type DocSection } from "@/lib/docs-data";
  ...
  return (
    <div className="my-6 rounded-2xl border border-border bg-foreground/[0.03] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/50 bg-foreground/[0.02]">
        ...
        <button
          onClick={handleCopy}
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] text-muted-foreground hover:text-foreground/70 hover:bg-foreground/[0.07] transition-all"
        >
  ...
  return (
    <div className="my-6 rounded-xl border border-border bg-foreground/[0.03] overflow-hidden">
      <div className="px-4 py-3 border-b border-border/50 bg-foreground/[0.02] flex items-center gap-2">
        <FileCode className="w-4 h-4 text-purple-400/70" />
...
function DocsSidebar({
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
            ...
            <div key={group.label} className="mb-4">
              ...
              <div className="space-y-0.5">
                {group.items.map((item) => {
                  ...
                  const Icon = docSections.find((s) => s.id === item.id)?.icon
                    ? iconMap[docSections.find((s) => s.id === item.id)!.icon!]
                    : BookOpen;
                  return (
                    <button
                      key={item.id}
                      ...
                      className={`flex items-center gap-2.5 w-full px-3 py-2 rounded-lg text-left text-[13px] transition-all ${
                        isActive
      ...
      <div className="p-4 border-t border-border">
        <p className="text-[10px] text-muted-foreground/50">FerrumEngine v1.0</p>
        <p className="text-[10px] text-muted-foreground/40 mt-0.5">MIT License</p>
      </div>
    </div>
  );
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex w-64 xl:w-72 flex-col border-r border-border bg-foreground/[0.02] h-screen sticky top-0 shrink-0">
        {sidebarContent}
      </aside>
      ...
      {mobileOpen && (
        ...
        {mobileOpen && (
          <aside
            className="fixed left-0 top-0 bottom-0 z-50 w-[280px] bg-[#0a0a0c] border-r border-border lg:hidden animate-[slideInLeft_0.3s_cubic-bezier(0.22,1,0.36,1)]"
          >
            {sidebarContent}
          </aside>
        )}
...
export function DocsView({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    document.title = `${activeSection.title} — FerrumEngine Docs`;
  }, [activeSection.title]);

  const Icon = iconMap[activeSection.icon] || BookOpen;
  ...
  return (
    <div className="flex min-h-screen bg-[#09090b]">
      ...
      <div className="flex-1 min-w-0">
        {/* Mobile header */}
        <div className="lg:hidden sticky top-0 z-30 flex items-center gap-3 px-4 h-14 border-b border-border bg-[#09090b]/80 backdrop-blur-xl">
          <button
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {activeSection.label && (
                      <Badge variant="outline" className="text-[10px] px-2 py-0.5 border-purple-500/30 text-purple-400/70">
                        {activeSection.label}
                      </Badge>
                    )}
                  </div>
