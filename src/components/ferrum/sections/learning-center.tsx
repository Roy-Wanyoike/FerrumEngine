...
import {
  ...
  ChevronRight, Clock, Layers, GraduationCap, Lightbulb,
  ArrowRight,
} from "lucide-react";
...
export function LearningCenter() {
  ...
  const expanded = paths.find((p) => p.id === expandedPath);
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        ...
        <Reveal>
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="ferrum-section-label text-xs font-semibold uppercase tracking-[0.15em] text-purple-400/70">Learning Center</p>
          </div>
        </Reveal>
        <Reveal delay={0.05}>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground tracking-tight max-w-3xl">
            Learn Interface Engineering
          </h1>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="text-lg text-muted-foreground/80 max-w-2xl leading-relaxed mt-5">
            Not just APIs — principles. These learning paths teach the engineering
            behind beautiful, performant, and accessible user interfaces.
          </p>
        </Reveal>
        ...
        <div className="mt-14 space-y-6">
          {paths.map((path, i) => {
            ...
            return (
              <Reveal key={path.id} delay={i * 0.05}>
                <div className="rounded-2xl border border-border/40 bg-foreground/[0.015] overflow-hidden transition-all duration-300">
                  ...
                  {/* Expanded modules */}
                  {isExpanded && (
                    <div className="border-t border-border/40 px-6 sm:px-8 pb-6 sm:pb-8 pt-4">
          ...
          <div className="mt-16 text-center">
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl border border-purple-500/15 bg-purple-500/[0.03]">
              ...
              <span className="text-sm font-medium text-foreground/70">
                More learning paths coming soon — AI-assisted, performance, and advanced VFX.
              </span>
            </div>
          </div>
        </Reveal>
