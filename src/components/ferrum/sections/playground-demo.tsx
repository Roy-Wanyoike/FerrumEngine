...
function generateCode(
  ...
  const cssComment = `/* Motion: ${motion} */`;
  const cssTransition = motion === "None"
    ? "  /* No motion */"
    : `  transition: ${motionTransitions[motion]};`;
  const effectCSS = getEffectCSSBlock(effect, depth);
...
function generateCSS(comp: string, motion: string, effect: string, depth: number, duration: number): string {
...
function generateReact(comp: string, motion: string, effect: string, depth: number, duration: number): string {
  const motionVal = motion.toLowerCase();
...
function generateVue(comp: string, motion: string, effect: string, depth: number, duration: number): string {
  const effectVal = effect.toLowerCase();
  const effectClass = `rc-${effectVal}`;
...
function generateSvelte(comp: string, motion: string, effect: string, depth: number, duration: number): string {
  const effectVal = effect.toLowerCase();
  const effectClass = `rc-${effectVal}`;
...
function generateWC(comp: string, motion: string, effect: string, depth: number, duration: number): string {
  const effectVal = effect.toLowerCase();
  const effectClass = `rc-${effectVal}`;
              ...
              <div className="border-b lg:border-b-0 lg:border-r border-border p-5 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={motion} onValueChange={setMotion}>
                    <SelectTrigger className="w-full h-9 text-sm bg-foreground/[0.03] border-border/60">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {motions.map((m) => (
